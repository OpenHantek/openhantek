...
SpectrumGenerator::SpectrumGenerator(const ::Settings::Scope *scope, const Settings *postprocessing)
...
void SpectrumGenerator::process(PPresult *result) {
    ...
    for (DataChannel &channelData : *result) {
        if (channelData.voltage.sample.empty() || !scope->channel(channelData.channelID)->spectrum()->visible()) {
